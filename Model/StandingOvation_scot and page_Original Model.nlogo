...
to set-cyclic-cond-1
  ...
  (foreach (n-values n-of-cycling-rows [ ?1 -> max-pycor - ?1 ]) (n-values n-of-cycling-rows [random 2])
    [ [?1 ?2] -> ask turtles with [(pycor = ?1) and (pxcor mod 2 = ?2)] [set standing? true] ])
...
to set-cyclic-cond-2
  ...
  let my-rd-list reduce [ [?1 ?2] -> sentence ?1 ?2 ] (map [ ?1 -> list ?1 ?1 ] (n-values n-of-pairs-of-cycling-rows [ ?1 -> ?1 mod 2 ]))
  let cycling-rows n-values (2 * n-of-pairs-of-cycling-rows) [ ?1 -> max-pycor - ?1 ]
